...
export function AllTestScreen({navigation}) {
    const [subjects, setSubjects] = useState([]);
    const [departmentEducations, setDepartmentEducations] = useState([]);
    ...
    const [selectedDE, setSelectedDE] = useState('0');
    const [openSubject, setOpenSubject] = useState(false);
    const [openDepartmentEducation, setOpenDepartmentEducation] = useState(false);
    ...
    useEffect(() => {
        ...
        referenceController.getAllTest(data => {
            setTests(data);
            setLoading(false);
        });
        ...
        mainController.getAllDepartmentEducation(data => {
            let temp = [{label: 'Tất cả', value: '0'}];
            for (let i = 0; i < data.length; i++) {
                temp.push({label: data[i].name, value: i + 1});
            }
            setDepartmentEducations(temp);
        })
    }, []);
    const wait = (timeout) => {
    ...
    const onRefresh = React.useCallback(() => {
        setRefreshing(true);
        referenceController.getAllTest(data => setTests(data));
    ...
    const filterData = (data, filters) => {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            (filters.department_education === '0' || data[i].department_education === departmentEducations[departmentEducations.findIndex(e => e.value == filters.department_education)].label)) {
                temp.push(data[i]);
            }
        }
        setResult(temp);
    ...
    useEffect(() => {
        filterData(tests, {subject: selectedSubject, department_education: selectedDE});
    }, [selectedSubject, selectedDE, tests]);
    ...
    return(
        <View style = {styles.container}>
            ...
            <View style = {{flex: 1}}>
                ...
                renderItem = {({item}) => <TestItem item = {item} onClick = {() => navigation.navigate('DetailTest', { testId: item._id })}/>}
                ...
                <View style = {{flex: 1}}>
                    <SafeAreaView style = {{backgroundColor: colorConfig.accentColor}}>
                        <View style = {styles.chatTool}>
                            ...
                            <TouchableOpacity onPress = {() => filterKeyword(tests, keyword)}>
                    ...
                    <View style={{backgroundColor: '#eeeeee', flex: 1}}>
                        ...
                        renderItem = {({item}) => <TestItem item = {item} onClick = {() => {setShowModal(false); navigation.navigate('DetailTest', { testId: item._id })}}/>}
                        removeClippedSubviews = {true} showsVerticalScrollIndicator = {false}
                        maxToRenderPerBatch = {20} initialNumToRender = {20}
